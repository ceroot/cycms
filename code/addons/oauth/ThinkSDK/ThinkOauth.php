...
namespace addons\oauth\ThinkSDK;
...
abstract class ThinkOauth
{
    ...
    public function __construct($token = null)
    {
        ...
        $config['app_key']      = config("think_sdk_{$this->Type}_app_key");
        $config['app_secret']   = config("think_sdk_{$this->Type}_app_secret");
        $config['app_callback'] = config("think_sdk_{$this->Type}_app_callback");

        //获取应用配置
        // $config = config("think_sdk_{$this->Type}");

        if (empty($config['app_key']) || empty($config['app_secret'])) {
            echo '请配置您申请的APP_KEY和APP_SECRET，还有回调地址';
            die;
